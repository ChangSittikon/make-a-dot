...
// 1. Update handleSaveRule to work with rulesList (since it saves to GEMINI.md, which is read into rules)
code = code.replace(
  /const newSkills = \[\.\.\.skillsList\];/g,
  'const newSkills = [...rulesList];' // keep the variable name newSkills for the API payload
);
code = code.replace(
  /setSkillsList\(newSkills\);/g,
  'setRulesList(newSkills);'
);
code = code.replace(
  /setSkillsList\(\[\.\.\.skillsList, \{ title: ruleTitle, desc: ruleDesc \}\]\);/g,
  'setRulesList([...rulesList, { title: ruleTitle, desc: ruleDesc }]);'
);

// 2. Replace the View Skills Modal header & content to include Tabs and add button at the bottom
const oldModalContent = `              <div className="p-5 overflow-y-auto flex-1 bg-gray-50 space-y-4">
                <button 
                  onClick={openAddRule}
...
const newModalContent = `              <div className="flex gap-2 px-5 py-3 bg-white border-b border-gray-100">
                <button 
...
// Replace the modal body (we need to find the exact block or use regex carefully)
