...
export async function GET() {
  try {
    ...
    const steps = nodes.map((node) => ({
      id: node.id,
      type: node.inputType || "TEXT", // TEXT | TEXTAREA | NUMBER | OPTIONS | MAGIC_SEARCH | DYNAMIC_REWARD | CASCADING_OPTIONS
      question: node.question || "",
      field: node.fieldName || "",
      suggestions: node.suggestions ? JSON.parse(node.suggestions) : undefined,
      // For OPTIONS: send options array
      options:
        node.inputType === "OPTIONS"
          ? node.options.map((opt) => ({
              label: opt.label,
              value: opt.vectorWeight
                ? JSON.parse(opt.vectorWeight).value
                : opt.label,
